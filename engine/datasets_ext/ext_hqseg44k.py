...
class LoadExtHQSeg44kTrainAnnotations(LoadAnnotations):
    ...
    def _load_seg_map(self, results):
        ...
        # if img_shape[:2] != gt_seg_map.shape:
        #     if img_shape[:2] == gt_seg_map.shape[::-1]:
        #         print_log(
        #             f'Warning: the image shape {img_shape[:2]} is different '
        #             f'from the segmentation map shape {gt_seg_map.shape}, '
        #             f'transpose and flip the segmentation map shape '
        #             f'to match the image shape.',
        #             logger='current',
        #             level=logging.WARNING)
        #         gt_seg_map = np.flip(gt_seg_map.transpose(), axis=0)
        #     else:
        #         raise ValueError(
        #             f'The image shape {img_shape[:2]} is different from '
        #             f'the segmentation map shape {gt_seg_map.shape}, '
        #             f'found in {results["seg_map_path"]}.')
...
class ExtHQSeg44kValDataset(BaseFewShotInterSegDataset):
    ...
    def __init__(self,
        ...
        self.max_hw_range = max_hw_range
    ...
    def load_data_list(self):
        ...
        if meta_root.is_file():
            ...
            for subdir in self.subdirs:
                ...
                for prefix in tqdm(sorted(img_files.keys())):
                    if prefix in prefixes:
                        with Image.open(str(img_files[prefix]), 'r') as img:
                            max_hw = max(np.array(img).shape[:2])
            ...
            barrier()
        if self.max_hw_range is not None:
            rmin, rmax = self.max_hw_range
            data_list = [d for d in data_list
                         if rmin < d['max_hw'] <= rmax]
        return data_list
...
class ExtHQSeg44kTrainDataset(BaseFewShotInterSegDataset):
    ...
    def load_data_list(self):
        ...
        if meta_root.is_file():
            ...
            barrier()
        valid_data_list = [
            sample for i, sample in enumerate(data_list)
            if i not in self.invalid_sample_indices
        ]
